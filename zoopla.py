...
def get_prop_api(use_api = False):
    ...
    if use_api == False:
        ...
        for page in range(1,num_pages+1): # start at 1 for first page, end at the last page
            if api_limit == 95:
                print(f"Have executed {api_limit} pages, going to sleep for an hour")
                time.sleep((60*60)+(60*5))
                api_limit = 0
            else:
                pass
            params = Zoopla_Config.params 
            params["page_num"] = page #update the page number to itterate through pages
            print(params)
            ...
            page_df.to_csv(Zoopla_Config.output_add, header=False,  mode='a')
